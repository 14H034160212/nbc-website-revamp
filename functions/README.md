...
`src/i18n/glossary.json` —— **翻译器和检查器读同一份文件**。只被告知不被约束的术语表是建议；
只被约束不被告知的是陷阱。
...
| | |
|---|---|
| **enforced**（9 项） | 与上下文无关的定译。草稿用了别的词就**失败 → 改走 PR** |
| **guidance**（8 项） | 该用哪个词取决于句子。只写进提示词，检查器不管 |

起因：模型把 separation of church and state 一次译成「政教分离」，一次译成「教会与国家分立」。
两个都对，但只有一个是这个站在用的词 —— **一份信仰声明不该因为工作流又跑了一次就改措辞。**

**校准规则：每一条 enforced 都必须让现有 345 条人工译文零失败。** 两个词因此被踢出强制层：

- `discipleship` —— 「discipleship **journey**」是「门徒之路」，不是「门徒训练」（那是课程）
- `home group` —— 独立标签用「家庭小组」，句中已有上下文时用「小组」。「青少年家庭小组」拗口，
  而「大家在家里以小组形式聚集」这句本身已经说了在家里

复数也踩过一次：`\bsermon\b` 匹配不到 `Sermons`，所以页面标题里的「证道」蒙混过关了。
现在支持规则复数；不规则复数（ministry/ministries）需要自己单独列一条，而不是假装正则能处理。
