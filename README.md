...
**英文永远先走。** 网站不会被翻译拖住，翻译也不会被网站催着赶。

### 机械检查能验什么

| 检查 | 抓什么 |
|---|---|
| 字符集 | 中文译文必须含汉字、不得含谚文；韩文必须含谚文；任何语言不得含西里尔字母或日文假名 |
| **英文残留** | 中文里留着 `games night`、`Bring a friend` 这种 —— **我自己犯过这个错**（`更多的connection`） |
| 保留词 | 邮箱、URL、电话必须原样出现；te reo 词和品牌名不得被译掉 |
| 长度比 | 只对 60 字以上的正文生效，阈值取自现有 329/326 条译文的实测分布（中文最低 0.19，韩文 0.30）。短字符串的比值毫无信息量 —— 「Throughout the Year」译成「全年」是 11%，完全正确 |
| 原样保留 | 与英文逐字相同才报错，且仅当原文里有小写英文单词 —— 人名、邮箱、网址本来就该不变 |

**校准方式：拿已有的 345 条人工译文跑，必须零失败。** 现在是零失败。

第一次真实运行就修正了一处校准：模型把 YouTube 译成了 `유튜브`，而检查器当时把
「保留词丢失」记成**备注**，于是放行了。页面上那个按钮写着 YouTube，词典其余各处也都保留原文
—— 这是机械可查、客观不一致的，所以已**升级为失败**（改走 PR 让人定，而不是丢掉整批工作）。
同一条规则顺带查出我自己在一处中文里丢了 `tamariki`。

### 不能验的，以及因此的分界

语域对不对、教义细节有没有走样、母语者读着自不自然 —— **没有任何自动检查能替代**。所以
`scripts/check-build.py` 里有一份 `REVIEW_REQUIRED`：

`/`、`/what-we-believe/`、`/services/`、`/who-we-are/`、`/first-visit/`

这些页面**检查全过也只开 PR**。一句关于青年营的中文稍微生硬，代价很小；一句关于洗礼的教义译错了，代价不一样。

顺带一提：**没有分支保护的话「自动 approve」是空的** —— 检查过了就直接合并更诚实，也少一层假动作。

### 自己跑

```sh
node scripts/check-translations.mjs                        # 全量校验
node scripts/check-translations.mjs --only translation-todo.json
ANTHROPIC_API_KEY=... node scripts/translate-new.mjs       # 起草
TRANSLATE_BASE_URL=http://localhost:11434/v1 \
  node scripts/translate-new.mjs                           # 用本地模型起草
```
